...
def error_incorrect_file():
    mb.showerror("Ошибка",
                 "Файл BMP некорректный. Принимаются только несжатые файлы, каждый пиксель в которых принимает значения #000000 или #FFFFFF")
    return 0

def error_bigsize():
    return mb.askyesno(
        title = "Предупреждение", 
        message = "Файл BMP слишком большой, его обработка может занять длительное время. Рекомендуется снизить разрешение дифракционной картинки. Вы уверены, что хотите загрузить его?")
...
def LoadBMP( filename ):
    ...
    В случае, если файл не соответствует указанным выше требованиям или он повреждён,
    вызывается функция error_incorrect_file() и функция LoadBMP возвращает (0, 0, 0).
    '''
    ...
    if (header[0] != 19778 or header[5] != 54 or header[7] != 40 or header[13] != 1 or header[14] != 24):
       file.close()
