...
def ProgramQuestion():
    mb.showinfo("Справка", "Программа позволяет численно рассчитать дифракционную картину на произвольном отверстии." +
                ...
                "Он может быть задан двумя способами:\n1) Предустановленный (Файл > Открыть пример);\n" +
...
def Sample( number, askwindow ):
    ...
    if (number.get() == 0):
        ...
        w, h, image = Example4()
    ObstacleLoaded(w, h, image, 555, 100, 2, 50, 100)
...
#Создаём окно
window = tk.Tk()
window.title("Фраунгофера дифракция на произвольном отверстии")
...
w, h, img = Example1()
ObstacleLoaded(w, h, img, 555, 100, 2, 50, 100)
